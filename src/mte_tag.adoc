...
`Zimte` defines a new instruction `netc` short for next instruction elide tag
check(s). Encoding for `netc` is taken from `C.MOP.3`, thus making `Zimte`
dependent on `C` and `Zcmop` extensions. If memory tagging is enabled, then
instruction `netc` sets `TTCD` to `TAG_CHECK_ELIDE` state. Thus any memory
accesses originating from subsequent instruction are elided for tag checks and
TTCD is cleared to `TAG_CHECK_ENFORCED` state again in the hart.

[NOTE]
=====
Compiler can insert `netc` as prefix on memory accesses on objects which are
local to a function and gain performance back. It will lead to a code size
growth (2 additional byte load/store) but stack tagging will anyways lead to
code size growth and it is expected that user enabling stack tagging has opted
into code size growth as a trade-off.
